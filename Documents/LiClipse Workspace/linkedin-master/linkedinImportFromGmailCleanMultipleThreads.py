...
# #         To log in
    ...
    with WebBus(browser) as bus:
        with WebBus(browser) as bus2:
    
            t1 = threading.Thread(target=bus.driver.get(GMAIL_URL))
            t2 = threading.Thread(target=bus2.driver.get(GMAIL_URL))
            t1.start()
            t2.start()
            t1.join()
            t1 = threading.Thread(login_into_gmail(bus.driver, username))
            t2.join()
            t2 = threading.Thread(login_into_gmail(bus2.driver, username))
            
            #             raw_input("get the contact into linkedin...")
            #         raw_input("Go on to linked in...")
            t1 = threading.Thread(bus.driver.get(LINKEDIN_URL))
            
            #         raw_input("Press Enter to continue...")
            time.sleep(1)
            
            t1 = threading.Thread(login_into_linkedin(bus.driver, username))
            ...
            path = "C:/Users/USUARIO/Documents/LiClipse Workspace/linkedin-master/Sample_50sales/2600/*.csv"
            ...
            #             raw_input("Delete contact from gmail...")
                t1 = threading.Thread(removeImportedEmailLinkedIn(bus))
                
                t2.join()
                t2 = threading.Thread(gmail_delete_contactsFromLabel(bus2.driver,filename))
                t2.join()
                t2 = threading.Thread(gmail_import_contacts(bus2.driver,filename))
                t2.join()
            ...
            #       edin.com/mynetwork/invitation-manager/sent/?filterCriteria=null']
                ...
                try:
                    t1 = threading.Thread(scrapDataImportOnLinkedInSourceFile(bus,directory,basefilenameNoExt))
            
            #                 raw_input("Get the link of LinkedIn...")
                    t1.join()
            ...
            #                 ORIGINAL VERSION
                    t1 = threading.Thread(scrapLinkedInAccountLinkSourceFile(bus,directory,basefilenameNoExt))
                    time.sleep(5)
                    t1 = threading.Thread(deleteLinkedInInvitations(bus))
            #                 raw_input("Delete the invitations...")
            #                 time.sleep(5)
                    try:
